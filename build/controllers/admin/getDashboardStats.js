...
exports.getDashboardStats = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    const [
        ...
        prisma_1.prisma.user.count({ where: { isVerified: true, isDeleted: false } }),
        prisma_1.prisma.user.count({ where: { isVerified: false, isDeleted: false } }),
        prisma_1.prisma.user.count({ where: { isDeleted: true } }),
        prisma_1.prisma.user.count({
            where: {
        ...
        prisma_1.prisma.author.count({ where: { status: "PENDING", isDeleted: false } }),
        prisma_1.prisma.author.count({ where: { status: "APPROVED", isDeleted: false } }),
        prisma_1.prisma.author.count({ where: { status: "REJECTED", isDeleted: false } }),
        prisma_1.prisma.author.count({ where: { isSuspended: true, isDeleted: false } }),
        prisma_1.prisma.author.count({
            where: {
        ...
        prisma_1.prisma.blog.count({ where: { isDeleted: true } }),
        prisma_1.prisma.blog.count({
            where: {
                createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
                isDeleted: false,
        ...
        prisma_1.prisma.commentLike.count(),
        prisma_1.prisma.blogRead.count(),
        prisma_1.prisma.comment.count({ where: { isDeleted: true } }),
        // === ADMIN QUERIES ===
